...
module "app_service_environments" {
  ...
  subnet_id                 = can(each.value.subnet.key) || can(each.value.subnet_key) ? local.combined_objects_networking[try(each.value.subnet.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.subnet.vnet_key, each.value.vnet_key)].subnets[try(each.value.subnet.key, each.value.subnet_key)].id : null
  ...
  subnet_name               = can(each.value.subnet_name) ? each.value.subnet_name : local.combined_objects_networking[try(each.value.subnet.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.subnet.vnet_key, each.value.vnet_key)].subnets[try(each.value.subnet.key, each.value.subnet_key)].name
...
module "app_service_environments_v3" {
  ...
  resource_group_name = can(each.value.resource_group.name) || can(each.value.resource_group_name) ? try(each.value.resource_group.name, each.value.resource_group_name) : local.combined_objects_resource_groups[try(each.value.resource_group.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group_key, each.value.resource_group.key)].name
  base_tags           = try(local.global_settings.inherit_tags, false) ? try(local.combined_objects_resource_groups[try(each.value.resource_group.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group.key, each.value.resource_group_key)].tags, {}) : {}
  subnet_id           = can(each.value.subnet_id) ? each.value.subnet_id : local.combined_objects_networking[try(each.value.subnet.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.subnet.vnet_key, each.value.vnet_key)].subnets[try(each.value.subnet.key, each.value.subnet_key)].id
