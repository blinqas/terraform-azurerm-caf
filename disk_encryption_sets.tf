module "disk_encryption_sets" {
  ...
  key_vault_key_id    = can(try(each.value.keyvault_key_id, each.value.keyvault_key.id)) ? try(each.value.keyvault_key_id, each.value.keyvault_key.id) : local.combined_objects_keyvault_keys[try(each.value.keyvault_key.lz_key, local.client_config.landingzone_key)][try(each.value.key_vault_key_key, each.value.key_vault_key.key)].id
