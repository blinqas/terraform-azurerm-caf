...
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    subnet_id                     = try(local.combined_objects_networking[try(each.value.vnet.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.vnet.vnet_key, each.value.vnet_key)].subnets[try(each.value.vnet.subnet_key, each.value.subnet_key)].id, null)
