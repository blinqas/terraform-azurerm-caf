...
locals {
  ...
  create_sshkeys = can(var.settings.virtual_machine_settings[var.settings.os_type].keyvault) && (local.os_type == "linux" || local.os_type == "legacy") && can(var.settings.public_key_pem_file) == false && can(var.settings.virtual_machine_settings[var.settings.os_type].admin_ssh_keys) == false
}
