resource "azurerm_dev_test_global_vm_shutdown_schedule" "enabled" {
  ...
  notification_settings {
    ...
    webhook_url     = can(var.settings.shutdown_schedule.notification_settings.webhook_url) ? var.settings.shutdown_schedule.notification_settings.webhook_url : try(var.settings.shutdown_schedule.notification_settings.webhook_url, null)
  }
}
