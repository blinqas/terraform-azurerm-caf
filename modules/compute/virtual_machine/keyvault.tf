locals {
  ...
  # try(var.keyvaults[var.settings.lz_key][var.settings.keyvault_key], var.keyvaults[try(var.settings.keyvault.lz_key, var.client_config.landingzone_key)][try(var.settings.keyvault.key, var.settings.keyvault_key)]) : null
}
