...
resource "azurecaf_name" "windows_computer_name" {
  ...
  name          = each.value.name
  resource_type = "azurerm_windows_virtual_machine"
  prefixes      = try(each.value.azurecaf_name.prefixes, var.global_settings.prefixes)
  random_length = try(each.value.azurecaf_name.random_length, var.global_settings.random_length)
  clean_input   = true
  passthrough   = try(each.value.azurecaf_name.passthrough, var.global_settings.passthrough)
  use_slug      = try(each.value.azurecaf_name.use_slug, var.global_settings.use_slug)
}
...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  computer_name                = try(each.value.computer_name, azurecaf_name.windows_computer_name[each.key].result)      # blinQ: Windows computer name can only be 15 characters, resource name can be 64. We need to be able to set the computer_name buypassing the azurecaf generated name.
...
locals {
  admin_username_windows = can(var.settings.virtual_machine_settings["windows"].admin_username_key) ? data.external.windows_admin_username.0.result.value : null
  admin_password_windows = can(var.settings.virtual_machine_settings["windows"].admin_password_key) ? data.external.windows_admin_password.0.result.value : null
}
