...
module "azurerm_firewall_policies" {
  ...
  resource_group = can(each.value.resource_group.id) ? each.value.resource_group.id : local.combined_objects_resource_groups[try(each.value.resource_group.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group.key, each.value.resource_group_key, )]
...
module "azurerm_firewall_policies_child" {
  ...
  resource_group = can(each.value.resource_group.id) ? each.value.resource_group.id : local.combined_objects_resource_groups[try(each.value.resource_group.lz_key, each.value.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group.key, each.value.resource_group_key)].id
  # resource_group = can(each.value.resource_group.id) ? each.value.resource_group.id : local.combined_objects_resource_groups[try(each.value.lz_key, each.value.resource_group.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group_key, each.value.resource_group.key)].id
